...
func scanner(ip string, port uint64) {
	...
	if host, ok := ps.PrimaryHost(ip); ok {
		hostOverride = host
	}
	//var iCntTimeOut = 0
	...
	for _, svc := range St_Identification_Port {
		if port == svc.Port {
			...
			dwSvc, resultEvent = SendIdentificationPacketFunction(data, ip, port, hostOverride)
			break
		}
	}
	...
	for i := 0; i < iPacketMask; i++ {
		...
		if i == 0 {
			...
			hostValue := ip
			if hostOverride != "" {
				hostValue = hostOverride
			}
			if port == 80 {
				szOption = fmt.Sprintf("%s%s\r\n\r\n", st_Identification_Packet[0].Packet, hostValue)
			} else {
				szOption = fmt.Sprintf("%s%s:%d\r\n\r\n", st_Identification_Packet[0].Packet, hostValue, port)
			}
		...
		dwSvc, resultEvent = SendIdentificationPacketFunction(packet, ip, port, hostOverride)
		if (dwSvc > UNKNOWN_PORT && dwSvc <= SOCKET_CONNECT_FAILED) || dwSvc == SOCKET_READ_TIMEOUT {
			Writer.Write(resultEvent)
			return
		}
...
func formatDisplayTarget(ip string, port uint64, hostOverride string) string {
	if hostOverride != "" {
		return fmt.Sprintf("%s:%d (%s)", hostOverride, port, ip)
	}
	return fmt.Sprintf("%s:%d", ip, port)
}

func SendIdentificationPacketFunction(data []byte, ip string, port uint64, hostOverride string) (int, *output.ResultEvent) {
	displayAddr := formatDisplayTarget(ip, port, hostOverride)
	...
	conn, err := net.DialTimeout("tcp", dialAddr, time.Duration(tout*1000)*time.Millisecond)
	if err != nil {
		// 端口是closed状态
	...
	if num > 0 {
		...
		if dwSvc > UNKNOWN_PORT && dwSvc < SOCKET_CONNECT_FAILED {
			...
			if szSvcName == "ssl/tls" || szSvcName == "http" {
				rst := Ghttp.GetHttpTitle(ip, hostOverride, Ghttp.HTTPorHTTPS, int(port))
				even.WorkingEvent = rst
				cert, err0 := Ghttp.GetCert(ip, hostOverride, int(port))
				if err0 != nil {
					cert = ""
				}
				even.Info.Cert = cert
