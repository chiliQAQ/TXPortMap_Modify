...
func ParseIpv4Range(ip string) (Range, error) {
	...
	if index != -1 {
		...
		start := strings.TrimSpace(ips[0])
		stop := strings.TrimSpace(ips[1])

		begin, err := ps.ParseIPv4(start)
		if err != nil {
			return Range{}, err
		}
		...
		endStr, err := normalizeRangeEnd(start, stop)
		if err != nil {
			return Range{}, err
		}

		end, err := ps.ParseIPv4(endStr)
		if err != nil {
			return Range{}, err
		}
...
func normalizeRangeEnd(start, end string) (string, error) {
	if ps.IsIP(end) {
		return end, nil
	}
	...
	if !strings.Contains(end, ".") {
		...
		if len(startSegments) != 4 {
			return "", errors.New("invalid start ip")
		}

		startLast, err := strconv.Atoi(startSegments[3])
		if err != nil {
			return "", err
		}

		endLast, err := strconv.Atoi(end)
		if err != nil {
			return "", err
		}

		if endLast < startLast {
			return "", errors.New("End ip is large than start ip")
		}

		if endLast < 0 || endLast > 255 {
			return "", errors.New("invalid ip range format")
		}

		return fmt.Sprintf("%s.%s.%s.%d", startSegments[0], startSegments[1], startSegments[2], endLast), nil
	}
